...
setwd("C:/Users/Jaffri/Desktop/Dataset and Algorithm - Jaffri (Stable)")
...
## BEGIN : STEP 1-COMPUTE INFORMATION CRITERION FOR INITIAL SET
  ...
  ICBEA[o]<-ICb(length(THRESHOLD),N,SSE,2)             #(m+1)-REGIME
## END : STEP 1 ######################################### 
...
for(i in 1: (length(THRESHOLDBEA) - 1)){
  ...
  V[j]<-ICb((length(THRESHOLD)-1),N,SSE,2)                 #m-REGIME (REMOVE ONE THRESHOLD)
}
