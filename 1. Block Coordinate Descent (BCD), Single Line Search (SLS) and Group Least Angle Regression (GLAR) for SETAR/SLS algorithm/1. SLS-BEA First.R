...
maxREP<-250
LAMBDAlength<-1
LAMBDAlist<-seq(0.1,0.01,length.out=LAMBDAlength)  #GENERATE LIST OF LAMBDAS
...
#############################################################################
set.seed(SEED)
ETA<-rmvnorm(n, mean=c(0), diag(1)) #Generate r.v \eta_{t}
ETA<-t(ETA)
#############################################################################
...
paraC1<-matrix(c(1),nrow=Q,byrow=TRUE)
paraC2<-matrix(c(0.6),nrow=Q,byrow=TRUE)
paraC3<-matrix(c(-1.0),nrow=Q,byrow=TRUE)
#AR parameter for FIRST regime 
paraT11<-matrix(c(-0.4),nrow=Q,byrow=TRUE)
#AR parameter for SECOND regime 
paraT21<-matrix(c(1),nrow=Q,byrow=TRUE)
#AR parameter for THIRD regime 
paraT31<-matrix(c(-0.2),nrow=Q,byrow=TRUE)

#HERE, SIGMA1-SIGMA3 and SD1-SD3 matrices can have different diagonal values for each regime
SIGMA1<-matrix(c(1),ncol=Q,byrow=TRUE); SD<-chol(SIGMA1); #Var-Cov Matrix (Variance for univariate case)
...
for (i in (p+1):((dim(ETA)[2]))) { 
  if(Y[,i-d]<=r1){                                       
  Y[,i]<-paraC1+(paraT11%*%(as.matrix(Y[,i-1])))+(SD%*%ETA[,i]) 
  }
  else if(Y[,i-d]> r1 && Y[,i-d]<=r2){
  Y[,i]<-paraC2+(paraT21%*%(as.matrix(Y[,i-1])))+(SD%*%ETA[,i]) 
  }else{
  Y[,i]<-paraC3+(paraT31%*%(as.matrix(Y[,i-1])))+(SD%*%ETA[,i]) 
  }
}
...
timeSTART<-Sys.time()
DATACOLL<-DATA3A(SamSize,d=1,r1= -0.8, r2= 0.5,SEED=REPLICATE)
...
save(LISTA,file=paste("4. SLS vs aBCD/1.SLS/750/1.First/SIMUBCDBEA.(",REPLICATE,").RData",sep = ""))
}
################################################################################################################################