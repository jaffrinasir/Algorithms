...
DATA3A<-function(n,d,r1,r2,r3,r4,r5,r6,r7,r8,C1,C2,C3,C4,C5,C6,C7,C8,C9,SEED){
...
paraC1<-matrix(c(C1),nrow=Q,byrow=TRUE)
paraC2<-matrix(c(C2),nrow=Q,byrow=TRUE)
paraC3<-matrix(c(C3),nrow=Q,byrow=TRUE)
paraC4<-matrix(c(C4),nrow=Q,byrow=TRUE)
paraC5<-matrix(c(C5),nrow=Q,byrow=TRUE)
paraC6<-matrix(c(C6),nrow=Q,byrow=TRUE)
paraC7<-matrix(c(C7),nrow=Q,byrow=TRUE)
paraC8<-matrix(c(C8),nrow=Q,byrow=TRUE)
paraC9<-matrix(c(C9),nrow=Q,byrow=TRUE)
#AR(2) parameter for FIRST regime 
paraT11<-matrix(c(-0.6),nrow=Q,byrow=TRUE)
paraT12<-matrix(c(0),nrow=Q,byrow=TRUE)
#AR(2) parameter for SECOND regime 
paraT21<-matrix(c(0.3),nrow=Q,byrow=TRUE)
paraT22<-matrix(c(0.9),nrow=Q,byrow=TRUE)
#AR(2) parameter for THIRD regime 
paraT31<-matrix(c(-0.9),nrow=Q,byrow=TRUE)
paraT32<-matrix(c(0),nrow=Q,byrow=TRUE)
#AR(2) parameter for FORTH regime 
paraT41<-matrix(c(0.7),nrow=Q,byrow=TRUE)
paraT42<-matrix(c(0.5),nrow=Q,byrow=TRUE)
#AR(2) parameter for FIFTH regime 
paraT51<-matrix(c(0.1),nrow=Q,byrow=TRUE)
paraT52<-matrix(c(0),nrow=Q,byrow=TRUE)
#AR(2) parameter for SIXTH regime 
paraT61<-matrix(c(-0.9),nrow=Q,byrow=TRUE)
paraT62<-matrix(c(0),nrow=Q,byrow=TRUE)
#AR(2) parameter for SEVENTH regime 
paraT71<-matrix(c(0.9),nrow=Q,byrow=TRUE)
paraT72<-matrix(c(0),nrow=Q,byrow=TRUE)
#AR(2) parameter for EIGHTH regime 
paraT81<-matrix(c(-0.8),nrow=Q,byrow=TRUE)
paraT82<-matrix(c(-0.2),nrow=Q,byrow=TRUE)
#AR(2) parameter for EIGHTH regime 
paraT91<-matrix(c(-1.1),nrow=Q,byrow=TRUE)
paraT92<-matrix(c(0),nrow=Q,byrow=TRUE)

#HERE, SIGMA1-SIGMA3 and SD1-SD3 matrices can have different diagonal values for each regime
SIGMA1<-matrix(c(1),ncol=Q,byrow=TRUE); SD<-chol(SIGMA1); #Var-Cov First regime
...
for (i in (p+1):((dim(ETA)[2]))) { 
  ...
  Y[,i]<-paraC4+(paraT41%*%(as.matrix(Y[,i-1])))+(paraT42%*%(as.matrix(Y[,i-2])))+(SD%*%ETA[,i])  
  }
  else if(Y[,i-d]> r4 && Y[,i-d]<=r5){
  Y[,i]<-paraC5+(paraT51%*%(as.matrix(Y[,i-1])))+(paraT52%*%(as.matrix(Y[,i-2])))+(SD%*%ETA[,i])  
  }
  else if(Y[,i-d]> r5 && Y[,i-d]<=r6){
  Y[,i]<-paraC6+(paraT61%*%(as.matrix(Y[,i-1])))+(paraT62%*%(as.matrix(Y[,i-2])))+(SD%*%ETA[,i])  
  }
  else if(Y[,i-d]> r6 && Y[,i-d]<=r7){
  Y[,i]<-paraC7+(paraT71%*%(as.matrix(Y[,i-1])))+(paraT72%*%(as.matrix(Y[,i-2])))+(SD%*%ETA[,i])  
  }
  else if(Y[,i-d]> r7 && Y[,i-d]<=r8){
  Y[,i]<-paraC8+(paraT81%*%(as.matrix(Y[,i-1])))+(paraT82%*%(as.matrix(Y[,i-2])))+(SD%*%ETA[,i])  
  }
  else{
  Y[,i]<-paraC9+(paraT91%*%(as.matrix(Y[,i-1])))+(paraT92%*%(as.matrix(Y[,i-2])))+(SD%*%ETA[,i])  
  }
}
  ...
  for(i4 in 1:SAMPLE){
    ...
    if(i4==1){
      ...
      if((2*norm(W,type="2")/SAMPLE) < LAMBDA){
        ...
        uvect<-uniroot.all(f2a, c(1e-10, 1e10))
...
timeSTART<-Sys.time()
DATACOLL<-DATA3A(SamSize,d=1,r1= -3.5, r2= -2.5 ,r3= -1.5, r4= -0.5,r5= 0.5, r6=1.5, r7=2.5,r8=3.5,
                 C1=-4.5,C2=2.5,C3=-2.0,C4=2.3,C5=1.0,C6=3.0,C7=1.6,C8=-0.5,C9=1.5, SEED=REPLICATE)
...
TRUETHR<-c(-3.5,-2.5,-1.5,-0.5,0.5,1.5,2.5,3.5)
...
save(LISTA,file=paste("4. SLS vs aBCD/1.SLS/750/3.Third/SIMUBCDBEA.(",REPLICATE,").RData",sep = ""))
}
################################################################################################################################