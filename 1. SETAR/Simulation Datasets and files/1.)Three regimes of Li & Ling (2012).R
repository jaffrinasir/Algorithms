#DATA FOR MODEL (3.1) IN CHAN ET AL.(2015), ADAPTED FROM LI AND LING (2012)
...
paraC1<-matrix(c(1),nrow=K,byrow=TRUE)
paraC2<-matrix(c(0.6),nrow=K,byrow=TRUE)
paraC3<-matrix(c(-1.0),nrow=K,byrow=TRUE)
#AR parameter for FIRST regime 
paraT11<-matrix(c(-0.4),nrow=K,byrow=TRUE)
#AR parameter for SECOND regime 
paraT21<-matrix(c(1),nrow=K,byrow=TRUE)
#AR parameter for THIRD regime 
paraT31<-matrix(c(-0.2),nrow=K,byrow=TRUE)
...
for (i in (p+1):((dim(Z)[2]))) { 
	if(Y[,i-d]<=r1){                                       
	Y[,i]<-paraC1+(paraT11%*%(as.matrix(Y[,i-1])))+(SD1%*%Z[,i])	
	}
	else if(Y[,i-d]> r1 && Y[,i-d]<=r2){
	Y[,i]<-paraC2+(paraT21%*%(as.matrix(Y[,i-1])))+(SD2%*%Z[,i])	
	}else{
	Y[,i]<-paraC3+(paraT31%*%(as.matrix(Y[,i-1])))+(SD3%*%Z[,i])	
	}
}
...
n=1200
DATACOLL<-DATA3A(n,d=1,r1= -0.8, r2= 0.5,SEED="NA")
...
### PLOTING TIME SERIES #################################################################
par(mfrow=c(1,2))
plot(NA, xlim=c(1,n), ylim=c(-5,6), main="First Series", ylab="", xlab="Time")
points(Y0[,1],col="red", type="l", pch=23,lty="dotted", cex=4)

plot(NA, xlim=c(1,n), ylim=c(-5,6), main="Rearranged observations", ylab="", xlab="Time")
