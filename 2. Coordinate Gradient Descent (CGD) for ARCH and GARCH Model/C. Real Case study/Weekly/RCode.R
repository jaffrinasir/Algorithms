...
library(ggplot2)
library(forecast)
ASX_Ordinary <- read_excel("C:/Users/Jaffri/Dropbox/PhD Research/4. Third Year/3. Thesis Write up/Simulation Studies-Rcode/B. Pure GARCH models/C. Real Case study/Weekly/WeekASXOrd.xlsx", col_types = c("date", "numeric", "numeric", "numeric", "numeric"))
library("ggplot2", lib.loc="~/R/win-library/3.5")
plot1<-ggplot(data = ASX_Ordinary, aes(x = Date, y = Close))+geom_line(color = "black", size = 1) + labs(title = "ASX Ordinary", y = "Weekly Closing Price", x="Time")
...
plot2<-ggplot(data = ASX_Return, aes(x = Date, y = Return))+geom_line(color = "red", size = 1) + labs(title = "ASX Ordinary Log Return", y="Log Return, Weekly Closing Price (%)", x="Time")
...
ggAcf(Return^2,lag.max = 60,lag=60, main="ACF of Squared ASX Ordinary Log Return")
ggPacf(Return^2,lag.max = 60,lag=60, main="PACF of Squared ASX Ordinary Log Return")
...
SigARCH <- read.csv("C:/Users/Jaffri/Dropbox/PhD Research/4. Third Year/3. Thesis Write up/Simulation Studies-Rcode/B. Pure GARCH models/C. Real Case study/Weekly/SigARCH.csv")
SigARCH<-SigARCH[,2]; 
Fit_ARCH<-data.frame(Date1[(V+2):length(Date1)],SigARCH[(V+1):length(SigARCH)]);
colnames(Fit_ARCH)[1]<-"Date"; colnames(Fit_ARCH)[2]<-"Fit"
SigGARCH <- read.csv("C:/Users/Jaffri/Dropbox/PhD Research/4. Third Year/3. Thesis Write up/Simulation Studies-Rcode/B. Pure GARCH models/C. Real Case study/Weekly/SigGARCH.csv")
SigGARCH<-SigGARCH[,2]; 
